...
export const DriverTimeline = () => {
    ...
    // Debounce the driver search query by 300ms
    ...
    // Filter drivers based on debounced search query
    const filteredDrivers = useMemo(() => {
        if (!debouncedDriverSearch.trim()) return drivers;
        return drivers.filter((driver) =>
            driver.name.toLowerCase().includes(debouncedDriverSearch.toLowerCase())
        );
    }, [drivers, debouncedDriverSearch]);
    ...
    return (
        <div className="bg-white rounded-lg shadow-xs border border-gray-200 mb-6 p-4">
            ...
            <div className="flex flex-wrap items-center justify-end gap-3.5 py-2 text-[11px] text-gray-500 font-medium select-none">
                <span className="flex items-center gap-1">
                    <span className="w-2.5 h-2.5 rounded-xs bg-blue-100 border border-blue-400"></span> Duty Start
                </span>
                <span className="flex items-center gap-1">
                    <span className="w-2.5 h-2.5 rounded-xs bg-blue-200 border border-blue-600"></span> Duty End
                </span>
                <span className="flex items-center gap-1">
                    <span className="w-2.5 h-2.5 rounded-xs bg-purple-600"></span> Pickup/Drop
                </span>
                <span className="flex items-center gap-1">
                    <span className="w-2.5 h-2.5 rounded-xs bg-amber-400"></span> Break
                </span>
                <span className="flex items-center gap-1">
                    <span className="w-2.5 h-2.5 rounded-xs bg-emerald-400"></span> Vehicle Change
                </span>
                <span className="flex items-center gap-1">
                    <span className="w-2.5 h-2.5 rounded-xs bg-gray-300"></span> Empty Leg
                </span>
            </div>
                                ...
                                <div className="w-60 shrink-0 flex items-center justify-between pr-4">
                                    <div className="flex items-center gap-2.5">
                                        ...
                                        <div>
                                            ...
                                            <span
                                                className={`text-[10px] px-1.5 py-0.2 rounded font-medium ${driver.status === 'Online'
